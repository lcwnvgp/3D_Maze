 ...
 */
 ...
 public:
   ...
   struct BVHNode {
     ...
     int firstPrimitive; // 第一个图元索引
     int primitiveCount; // 图元数量
   ...
   struct BVHAccel {
     ...
     void build(const std::vector<Triangle>& triangles, int objectIndex);
     void subdivide(int nodeIdx, int start, int end, int depth);
     bool intersect(const glm::vec3& point, float radius, glm::vec3& contactN, float& penetration) const;
   };
   ...
   // 物理引擎相关变量
   glm::vec3 ballPos{0.0f};  // 球体位置
   glm::vec3 ballVel{0.0f};  // 球体速度
   float ballRadius{1.0f};   // 球体半径
   const glm::vec3 GRAV{0.0f, -9.81f, 0.0f};  // 重力加速度
   float μ{0.05f};  // 摩擦系数
   float e{0.15f};  // 弹性系数
   float subDt{0.0f};  // 子步长
