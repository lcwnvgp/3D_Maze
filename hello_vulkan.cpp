 ...
 */
 ...
 void HelloVulkan::updateUniformBuffer(const VkCommandBuffer& cmdBuf)
 {
   ...
   auto     uboUsageStages = VK_PIPELINE_STAGE_VERTEX_SHADER_BIT | VK_SHADER_STAGE_RAY_TRACING_SHADER_BIT_KHR;
 ...
 void HelloVulkan::loadModel(const std::string& filename, glm::mat4 transform)
 {
   ...
   if (filename.find("maze.obj") != std::string::npos) {
     ...
     mazeBVH.build(mazeTris, 0);
     auto endTime = std::chrono::high_resolution_clock::now();
     debugInfo.buildTime = std::chrono::duration<float>(endTime - startTime).count();
     debugInfo.bvhNodes = mazeBVH.nodes.size();
     debugInfo.totalTriangles = mazeTris.size();
   }
 ...
 void HelloVulkan::BVHAccel::build(const std::vector<Triangle>& triangles, int objectIndex) {
   ...
   root.primitiveCount = triangles.size();
   root.leftChild = -1;
   root.rightChild = -1;
   
   // 计算根节点的AABB
   for(const auto& tri : triangles) {
     root.bounds.expand(tri.a);
     root.bounds.expand(tri.b);
     root.bounds.expand(tri.c);
   }
   ...
   // 递归细分
   subdivide(0, 0, triangles.size(), 0);
 }
 
 void HelloVulkan::BVHAccel::subdivide(int nodeIdx, int start, int end, int depth) {
   ...
   for(const auto& center : centers) {
     centerBounds.expand(center);
   }
   
   // 选择最长的轴进行分割
   glm::vec3 extent = centerBounds.max - centerBounds.min;
   int axis = 0;
   ...
   leftNode.firstPrimitive = start;
   leftNode.primitiveCount = mid - start;
   rightNode.firstPrimitive = mid;
   rightNode.primitiveCount = end - mid;
   
   // 计算子节点的AABB
   for(int i = start; i < mid; i++) {
     const Triangle& tri = primitives[i].triangle;
     leftNode.bounds.expand(tri.a);
     leftNode.bounds.expand(tri.b);
     leftNode.bounds.expand(tri.c);
   }
   
   for(int i = mid; i < end; i++) {
     const Triangle& tri = primitives[i].triangle;
     rightNode.bounds.expand(tri.a);
     rightNode.bounds.expand(tri.b);
     rightNode.bounds.expand(tri.c);
   }
   
   // 添加子节点
   leftNode.leftChild = -1;
   leftNode.rightChild = -1;
   rightNode.leftChild = -1;
   rightNode.rightChild = -1;
   ...
   // 递归细分子节点
   subdivide(node.leftChild, start, mid, depth + 1);
   subdivide(node.rightChild, mid, end, depth + 1);
 }
 
 bool HelloVulkan::BVHAccel::intersect(const glm::vec3& point, float radius, glm::vec3& contactN, float& penetration) const {
   std::lock_guard<std::mutex> lock(mutex);
   
   // 使用栈进行迭代遍历
   std::stack<int> stack;
   stack.push(rootNodeIdx);
   ...
   while(!stack.empty()) {
     ...
     // 检查AABB是否与球体相交
     glm::vec3 closest = glm::clamp(point, node.bounds.min, node.bounds.max);
     float dist2 = glm::dot(closest - point, closest - point);
     ...
     if(node.isLeaf()) {
       ...
       for(int i = 0; i < node.primitiveCount; i++) {
         const Primitive& prim = primitives[node.firstPrimitive + i];
         const Triangle& tri = prim.triangle;
         
         glm::vec3 p = closestPointTriangle(point, tri.a, tri.b, tri.c);
 ...
 // 修改碰撞检测逻辑,使用BVH
 void HelloVulkan::updatePhysics(float dt) {
   // 计算子步长
   subDt = dt / 200.0f;  // 使用200个子步长
   ...
   // 使用BVH进行碰撞检测
   if(mazeBVH.intersect(next, ballRadius, contactN, penetration)) {
     collided = true;
   }
   
   auto endTime = std::chrono::high_resolution_clock::now();
   debugInfo.queryTime = std::chrono::duration<float>(endTime - startTime).count();
 ...
 void HelloVulkan::renderUI() {
   ...
   // 显示相机位置
   glm::vec3 cameraPos = CameraManip.getCamera().eye;
   ...
   // 可调节的物理参数
   ImGui::SliderFloat("Friction (μ)", &μ, 0.0f, 1.0f);
   ImGui::SliderFloat("Restitution (e)", &e, 0.0f, 1.0f);
   
   // BVH调试信息
   if(ImGui::CollapsingHeader("BVH Debug Info")) {
     ImGui::Text("Query Time: %.3f ms", debugInfo.queryTime * 1000.0f);
     ImGui::Text("Nodes Traversed: %d", debugInfo.nodesTraversed);
     ImGui::Text("Triangles Checked: %d", debugInfo.trianglesChecked);
   }
